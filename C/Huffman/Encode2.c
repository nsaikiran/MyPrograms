...
void writeBufferedBits( unsigned char*const bitString , int INIT,int FINISH) {
	...
	if ( /*FLAGS &*/ INIT )
		string	= (unsigned char*)calloc(BUFSIZ,sizeof(unsigned char));
	if ( /*FLAGS &*/ FINISH ) { // no more data. flush
		// code for ending
		string[byte+1]='\0';// or 0 will do.
	//	fprintf(oFile,"%s %d",string,index);
		}
	if ( !bitString ) return;
	unsigned temp;
	for ( int var	= 0; var < strlen(bitString) ; ++var ) {
		...
		index++;
		if ( !(index % 8) ) {
			if (byte < BUFSIZ - 2) byte++;
			else {
				//printf("written1 %d\n",var);
				string[BUFSIZ - 1] = '\0'; //or 0 will do.
			//	fputs(string,ofile);
				...
				//clear the string
				for ( int flag = 0; flag < BUFSIZ;++flag) string[flag]^=string[flag];
				index	= 0;
				byte	= 0;
				//printf("written1 %s %u\n",string,index);
